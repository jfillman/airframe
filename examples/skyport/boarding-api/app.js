...
function createApp({ store, version = pkg.version, lookup = lookupFlight, flights = 'built-in', events = { mode: () => 'off', received: () => 0 }, publicDir = path.join(__dirname, 'public') }) {
    ...
    try {
      ...
      if (url.pathname === '/api/whoami') {
        return json(res, 200, {
          ...
          cache: store.mode(), flights, events: events.mode(), eventsReceived: events.received(),
        }, { Connection: 'close' });
      }
