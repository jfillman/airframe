...
# Maven on a JVM under emulation is very slow (12+ minutes and still running, versus ~5 minutes for
# the whole native arm64 build). Java bytecode is the same on both architectures, so compile once,
